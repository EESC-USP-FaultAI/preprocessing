...
def plotST(*args):
    ...
    axs["inicio"].plot([timestep*i for i in range(int(start*ppc*60)-int(ppc/2), int(start*ppc*60)+int(ppc/2))],
                       amp[1][int(start*ppc*60)-int(ppc/2): int(start*ppc*60)+int(ppc/2)])
    axs["inicio"].plot([timestep*i for i in range(int(start*ppc*60)-int(ppc/2), int(start*ppc*60)+int(ppc/2))],
                            signal[int(start*ppc*60)-int(ppc/2): int(start*ppc*60)+int(ppc/2)], alpha=0.3)
    ...
    if not ('circuit' in title):
        ...
        axs["fim"].plot([timestep * i for i in range(int(end * ppc * 60) - int(ppc/2), int(end * ppc * 60) + int(ppc/2))],
                        amp[1][int(end * ppc * 60) - int(ppc/2): int(end * ppc * 60) + int(ppc/2)])
        axs["fim"].plot([timestep * i for i in range(int(end * ppc * 60) - int(ppc/2), int(end * ppc * 60) + int(ppc/2))],
                           signal[int(end * ppc * 60) - int(ppc/2): int(end * ppc * 60) + int(ppc/2)], alpha=0.3)
