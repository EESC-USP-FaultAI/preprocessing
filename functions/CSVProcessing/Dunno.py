...
def data_selection(data_file_path, meas, meas_file_path="medicoes.csv", single_phase=None):
    ...
    Parameters:
    - data_file_path: Path to csv with voltage and current data
    - meas: measurement name based on "medicoes.csv" file.
    - meas_file_path: Path to csv file "medicoes.csv" in your computer.
    - single_phase: select the phase (A, B or C) to extract single phase data. If None returns three-phase data (default=None).
...
def case_selection(data_folder_path, meas="Vmed_B1", meas_file_path="medicoes.csv", single_phase=None, scenarios_file_path="Cenarios_inic.csv", resistence=None, fault_type=None, angle=None, local=None):
    ...
    Parameters:
    - data_folder_path: Path to folder with csv files of voltage and current data
    - meas: measurement name based on "medicoes.csv" file.
    - meas_file_path: Path to csv file "medicoes.csv" in your computer.
    - single_phase: select the phase (A, B or C) to extract single phase data. If None returns three-phase data (default=None).
    - scenarios_file_path: Path to csv file "Cenarios_inic.csv" in your computer
    - resistence: Desired value of resistence evaluated in the scenario.
    - fault_type: Desired fault_type evaluated in the scenario.
    - angle:Desired value of incidence angle evaluated in the scenario.
    - local:Desired local evaluated in the scenario.
